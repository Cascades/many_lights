...
float geo_term(in int pbt_light_index, in vec3 shading_pos, in vec3 shading_normal)
{
    ...
    min_dist = mix(0.0000001, min_dist, step(0.0001, min_dist));
...
float mat_term(in int pbt_light_index, in vec3 shading_pos, in vec3 shading_ambient, in vec3 shading_diffuse, in float shading_specular, in vec3 shading_normal)
{
	...
	diff = max(diff, dot(shading_normal, normalize(vec3(input_ssbo.pbt[pbt_light_index].bb.max_bounds.x, input_ssbo.pbt[pbt_light_index].bb.min_bounds.y, input_ssbo.pbt[pbt_light_index].bb.min_bounds.z) - shading_pos)));
	diff = max(diff, dot(shading_normal, normalize(vec3(input_ssbo.pbt[pbt_light_index].bb.max_bounds.x, input_ssbo.pbt[pbt_light_index].bb.min_bounds.y, input_ssbo.pbt[pbt_light_index].bb.max_bounds.z) - shading_pos)));
	diff = max(diff, dot(shading_normal, normalize(vec3(input_ssbo.pbt[pbt_light_index].bb.max_bounds.x, input_ssbo.pbt[pbt_light_index].bb.max_bounds.y, input_ssbo.pbt[pbt_light_index].bb.min_bounds.z) - shading_pos)));
	diff = max(diff, dot(shading_normal, normalize(vec3(input_ssbo.pbt[pbt_light_index].bb.max_bounds.x, input_ssbo.pbt[pbt_light_index].bb.max_bounds.y, input_ssbo.pbt[pbt_light_index].bb.max_bounds.z) - shading_pos)));
